...
export default async function DashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-10">
      {/* Header */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 bg-[#1a1a1a] p-8 md:p-12 rounded-3xl border border-white/5 shadow-2xl relative overflow-hidden">
        {/* Decorative elements */}
        ...
        <Link href="/dashboard/invitations/new" className="relative z-10 w-full md:w-auto">
          <Button leftIcon={<Plus size={18} />} className="w-full md:w-auto py-4 px-8 text-sm tracking-widest uppercase shadow-2xl bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] text-black font-bold">
            Buat Undangan Baru
          </Button>
        </Link>
      </div>
      ...
      <div>
        ...
        {!invitations || invitations.length === 0 ? (
          <div className="bg-[#1a1a1a] p-16 text-center border border-white/5 rounded-3xl flex flex-col items-center shadow-lg">
            ...
            <Link href="/dashboard/invitations/new">
              <Button leftIcon={<Plus size={18} />} className="bg-white/10 text-white hover:bg-white/20 border-white/20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {invitations.map((inv) => (
              <div key={inv.id} className="bg-[#1a1a1a] border border-white/5 rounded-3xl flex flex-col overflow-hidden group shadow-lg transition-transform hover:-translate-y-1">
                <div className="p-8 pb-6 flex-1 flex flex-col relative">
                  {/* Decorative faint heart */}
                  <Heart className="absolute right-6 top-6 text-white/5" size={80} />
                  
                  <div className="relative z-10">
                    <Badge variant={inv.is_published ? "success" : "warning"} className="mb-6 px-3 py-1.5 shadow-sm bg-black/40 backdrop-blur-md border border-white/10">
                      {inv.is_published ? (
                    ...
                    <h3 className="font-serif text-2xl text-white mb-6 leading-snug">
                      {inv.groom_name} <span className="text-[var(--color-primary)] italic">&amp;</span> {inv.bride_name}
                    </h3>
                    
                    <div className="space-y-3 mb-6">
                      <div className="flex items-center gap-3 text-sm text-white/60">
                        <div className="w-8 h-8 rounded-full bg-black/40 flex items-center justify-center text-[var(--color-primary)] border border-white/5">
                          <Calendar size={14} />
                        </div>
                ...
                <div className="border-t border-white/5 p-4 bg-black/20 flex items-center justify-between gap-3">
                  <Link href={`/dashboard/invitations/${inv.id}`} className="flex-1">
                    <Button variant="secondary" className="w-full justify-center bg-white/10 hover:bg-white/20 text-white border-none">
                      Kelola
                    </Button>
                  </Link>
                  <a
                    ...
                    className="w-12 h-12 flex items-center justify-center rounded-full bg-white/10 hover:bg-[var(--color-primary)] border border-white/10 text-white transition-all shadow-sm group-hover:border-[var(--color-primary)]"
