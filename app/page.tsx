  ...
  return (
    <main className="min-h-screen text-[var(--color-text)]">
      ...
      <section className="relative pt-40 pb-20 px-6 min-h-[90vh] flex flex-col items-center justify-center text-center overflow-hidden">
        {/* Glow Effects */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-[var(--color-gold-600)] rounded-full blur-[120px] opacity-[0.07] pointer-events-none animate-glow origin-center" />
        
        <div className="relative z-10 max-w-4xl mx-auto animate-fade-up">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-[var(--color-border-gold)] bg-[rgba(212,175,55,0.05)] text-[var(--color-gold-400)] text-xs font-semibold tracking-widest uppercase mb-8 hover:bg-[rgba(212,175,55,0.1)] transition-all duration-300 cursor-default">
            <Sparkles size={14} className="animate-pulse" />
            Premium Digital Invitation
          </div>
          
          <h1 className="font-serif text-5xl md:text-7xl mb-6 leading-tight animate-fade-up delay-100">
            Bagikan Momen Bahagia dengan <span className="text-gold italic drop-shadow-lg">Elegan & Sempurna</span>
          </h1>
          
          <p className="text-[var(--color-text-muted)] text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed animate-fade-up delay-200">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 animate-fade-up delay-300">
            <Link href="/register" className="btn-primary w-full sm:w-auto text-base px-8 py-4 group">
              Mulai Buat Undangan <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
            </Link>
            <Link href="#features" className="btn-secondary w-full sm:w-auto text-base px-8 py-4 hover:scale-105 transition-transform">
              Lihat Fitur
            </Link>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className={`glass-panel-gold p-8 rounded-2xl animate-fade-up hover:-translate-y-2 transition-all duration-500 hover:shadow-2xl hover:shadow-[var(--color-primary-light)] border border-transparent hover:border-[var(--color-border-gold)]`} style={{ animationDelay: `${(i+1)*150}ms` }}>
                <div className="w-12 h-12 rounded-full bg-[rgba(212,175,55,0.1)] flex items-center justify-center mb-6 border border-[var(--color-border-gold)] group-hover:scale-110 transition-transform">
                  {feat.icon}
                </div>
      ...
      <section className="py-24 px-6 border-t border-[var(--color-border)] relative overflow-hidden">
        <div className="absolute inset-0 bg-gold-gradient opacity-[0.05] animate-pulse-soft" />
        
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          <h2 className="font-serif text-4xl md:text-5xl mb-6 animate-fade-up">Mulai Kisah Anda Hari Ini</h2>
          <p className="text-[var(--color-text-muted)] text-lg mb-10 animate-fade-up delay-100">
            Bergabung dengan ratusan pasangan lainnya dan buat undangan yang tak terlupakan.
          </p>
          <div className="animate-fade-up delay-200">
            <Link href="/register" className="btn-primary text-base px-10 py-4 hover:scale-105 transition-transform shadow-[0_0_20px_rgba(212,175,55,0.2)] hover:shadow-[0_0_30px_rgba(212,175,55,0.4)]">
